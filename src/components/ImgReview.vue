  ...
  >
    ...
    <div class="tip-wrapper" v-if="imgReviewVisible">
      ...
      <div class="tool-wrapper">
        ...
        <i class="item el-icon-refresh-right el-icon-s-custom" title="设为头像" @click="setHeadImg"></i>
        <a
...
<script>
import {setAvatar} from "@/request/user";
...
export default {
  ...
  computed: {
    ...
    // 当前显示的图片扩展名
    activeExtendName() {
      return this.imgReviewList[this.activeIndex].extension
    },
    activeFileId() {
      return this.imgReviewList[this.activeIndex].id
    },
  ...
  methods: {
    ...
    setHeadImg() {
      ...
      setAvatar(data).then(
        ...
        console.error(err)
        this.$message.error("头像设置失败，请稍后再试！")
      })
    }
  }
}
